...
/**
 * Format a value as a Danish number with comma decimal separator and two decimals.
 * Leaves non-numeric values untouched.
 */
export function formatNumberDK(value) {
  if (value === null || value === undefined) return '';
  // Handle numeric strings by replacing comma to dot for parsing
  const raw = typeof value === 'string' ? value.replace(',', '.') : value;
  const num = typeof raw === 'number' ? raw : parseFloat(raw);
  if (isNaN(num)) return value; // not a number, return original
  // Use Intl for proper formatting (will insert thousand separators if necessary)
  return new Intl.NumberFormat('da-DK', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(num);
}

/**
 * Helper to format only numeric-like cells while leaving others.
 */
export function formatIfNumericDK(cell) {
  if (cell === null || cell === undefined) return '';
  if (typeof cell === 'number') return formatNumberDK(cell);
  if (typeof cell === 'string') {
    // Trim and detect numeric (allow comma or dot)
    const trimmed = cell.trim();
    if (trimmed === '') return '';
    // Regex: optional sign, digits, optional decimal part with . or ,
    if (/^[+-]?\d+(?:[\.,]\d+)?$/.test(trimmed)) {
      return formatNumberDK(trimmed);
    }
    return cell; // not numeric string
  }
  return cell; // other types
}
